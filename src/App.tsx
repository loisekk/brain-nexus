...
import { fetchOurGraph } from './lib/local-graph-adapter';
...
const AppContent = () => {
  ...
  // ── Local mode: load from static JSON (no backend needed) ──────────────────
  // Default behavior when no ?server or ?project param is present:
  // loads from /knowledge-graph.json (put your graph in public/).
  // Override via ?data=<url> or force original onboarding via ?onboarding.
  const localLoadRan = useRef(false);
  useEffect(() => {
    if (localLoadRan.current || autoConnectRan.current) return;
    const params = new URLSearchParams(window.location.search);
    const dataUrl = params.get('data');
    const onboarding = params.get('onboarding');
    if (onboarding) return;
    localLoadRan.current = true;

    const loadUrl = dataUrl || '/knowledge-graph.json';

    setProgress({
      phase: 'extracting',
      percent: 10,
      message: 'Loading knowledge graph...',
      detail: `Fetching from ${loadUrl}`,
    });
    setViewMode('loading');

    (async () => {
      try {
        const graph = await fetchOurGraph(loadUrl);
        setGraph(graph);
        setGraphMode('full');
        setProjectName('brain-nexus');
        setCurrentRepo('brain-nexus');
        setViewMode('exploring');
        setProgress(null);
      } catch (err) {
        console.error('Local graph load failed:', err);
        setProgress({
          phase: 'error',
          percent: 0,
  ...
  }, [setGraph, setGraphMode, setProjectName, setCurrentRepo, setProgress, setViewMode]);
  ...
  useEffect(() => {
    ...
    if (!serverBaseUrl) return; // Skip heartbeat in local/offline mode
