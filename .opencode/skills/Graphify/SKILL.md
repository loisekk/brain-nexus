...
EXCLUDE_DIRS = {'conda', 'venv', '.venv', 'node_modules', '__pycache__', '.git', '.tox', 'site-packages', 'Lib', 'lib'}
...
for file_type in result.get('files', {}):
    result['files'][file_type] = [f for f in result['files'][file_type] if not should_exclude(f)]
result['total_files'] = sum(len(v) for v in result['files'].values())
print(json.dumps(result, ensure_ascii=False))
" > graphify-out/.graphify_detect.json
```

**Excluded directories:** `conda`, `venv`, `.venv`, `node_modules`, `__pycache__`, `.git`, `.tox`, `site-packages`, `Lib`, `lib` — these are filtered out automatically to prevent indexing dependency/stdlib files.
